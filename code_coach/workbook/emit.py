...
def solution(language: str, shape: str, args: dict) -> str | None:
    ...
    if emit_cpp2.handles(shape):
        return emit_cpp2.solution(language, shape, args)
    if emit_js9.handles(shape):
        return emit_js9.solution(language, shape, args)
    if emit_ts7.handles(shape):
        return emit_ts7.solution(language, shape, args)
    if emit_rust.handles(shape):
        return emit_rust.solution(language, shape, args)
    if emit_c.handles(shape):
        return emit_c.solution(language, shape, args)
    if emit_graph.handles(shape):
        return emit_graph.solution(language, shape, args)
    if emit_nodes.handles(shape):
        return emit_nodes.solution(language, shape, args)
    if emit_algo4.handles(shape):
        return emit_algo4.solution(language, shape, args)
    ...
    if emit_algo2.handles(shape):
        return emit_algo2.solution(language, shape, args)
    if emit_algo.handles(shape):
        return emit_algo.solution(language, shape, args)
    if emit_ts6.handles(shape):
        return emit_ts6.solution(language, shape, args)
    if emit_ts5.handles(shape):
        return emit_ts5.solution(language, shape, args)
    ...
    if emit_ts2.handles(shape):
        return emit_ts2.solution(language, shape, args)
    if emit_ts.handles(shape):
        return emit_ts.solution(language, shape, args)
    if emit_js8.handles(shape):
        return emit_js8.solution(language, shape, args)
    if emit_js7.handles(shape):
        return emit_js7.solution(language, shape, args)
    if emit_js6.handles(shape):
        return emit_js6.solution(language, shape, args)
    ...
    if emit_js3.handles(shape):
        return emit_js3.solution(language, shape, args)
    if emit_js2.handles(shape):
        return emit_js2.solution(language, shape, args)
    if emit_js.handles(shape):
        return emit_js.solution(language, shape, args)
    if emit_python21.handles(shape):
        return emit_python21.solution(language, shape, args)
    if emit_python20.handles(shape):
        return emit_python20.solution(language, shape, args)
    if emit_python19.handles(shape):
        return emit_python19.solution(language, shape, args)
    if emit_python18.handles(shape):
        return emit_python18.solution(language, shape, args)
    if emit_python17.handles(shape):
        return emit_python17.solution(language, shape, args)
    if emit_python16.handles(shape):
        return emit_python16.solution(language, shape, args)
    if emit_python15.handles(shape):
        return emit_python15.solution(language, shape, args)
    if emit_python14.handles(shape):
        return emit_python14.solution(language, shape, args)
    if emit_python13.handles(shape):
        return emit_python13.solution(language, shape, args)
    if emit_python12.handles(shape):
        return emit_python12.solution(language, shape, args)
    if emit_python11.handles(shape):
        return emit_python11.solution(language, shape, args)
    if emit_python10.handles(shape):
        return emit_python10.solution(language, shape, args)
    if emit_python9.handles(shape):
        return emit_python9.solution(language, shape, args)
    if emit_python8.handles(shape):
        return emit_python8.solution(language, shape, args)
    if emit_python7.handles(shape):
        return emit_python7.solution(language, shape, args)
    if emit_python6.handles(shape):
        return emit_python6.solution(language, shape, args)
    if emit_python5.handles(shape):
        return emit_python5.solution(language, shape, args)
    if emit_python4.handles(shape):
        return emit_python4.solution(language, shape, args)
    if emit_python3.handles(shape):
        return emit_python3.solution(language, shape, args)
    if emit_python2.handles(shape):
        return emit_python2.solution(language, shape, args)
...
def all_shape_ids() -> tuple[str, ...]:
    ...
    return (
        ...
        + emit_python2.SHAPE_IDS
        + emit_python3.SHAPE_IDS
        + emit_python4.SHAPE_IDS
        + emit_python5.SHAPE_IDS
        + emit_python6.SHAPE_IDS
        + emit_python7.SHAPE_IDS
        + emit_python8.SHAPE_IDS
        + emit_python9.SHAPE_IDS
        + emit_python10.SHAPE_IDS
        + emit_python11.SHAPE_IDS
        + emit_python12.SHAPE_IDS
        + emit_python13.SHAPE_IDS
        + emit_python14.SHAPE_IDS
        + emit_python15.SHAPE_IDS
        + emit_python16.SHAPE_IDS
        + emit_python17.SHAPE_IDS
        + emit_python18.SHAPE_IDS
        + emit_python19.SHAPE_IDS
        + emit_python20.SHAPE_IDS
        + emit_python21.SHAPE_IDS
        + emit_js.SHAPE_IDS
        + emit_js2.SHAPE_IDS
        + emit_js3.SHAPE_IDS
        + emit_js4.SHAPE_IDS
        ...
        + emit_sql.SHAPE_IDS
        + emit_algo4.SHAPE_IDS
        + emit_nodes.SHAPE_IDS
        + emit_graph.SHAPE_IDS
        + emit_c.SHAPE_IDS
        + emit_rust.SHAPE_IDS
        + emit_ts7.SHAPE_IDS
        + emit_js9.SHAPE_IDS
