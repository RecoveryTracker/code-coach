"""The second set of Dart code magnets: the same rules as tests/test_magnets.py.
...
These read DART_MAGNETS_2 directly rather than through `magnets()`, so
...
REGISTERED = all(magnet(m.id) is m for m in DART_MAGNETS_2)
NOT_REGISTERED = "DART_MAGNETS_2 is not in magnets() yet"
...
def _find(magnet_id: str) -> Magnet:
    return next(m for m in DART_MAGNETS_2 if m.id == magnet_id)


class ShapeTests(unittest.TestCase):
    def test_there_are_six_in_one_family(self) -> None:
        self.assertEqual(len(DART_MAGNETS_2), 6)
        for m in DART_MAGNETS_2:
    ...
    def test_ids_are_unique(self) -> None:
        ids = [m.id for m in DART_MAGNETS_2]
    ...
    def test_no_id_is_taken_by_another_puzzle(self) -> None:
        ...
        mine = {m.id for m in DART_MAGNETS_2}
        others = [
            m.id for m in (*magnets(), *DART_MAGNETS)
            if not any(m is d for d in DART_MAGNETS_2)
        ]
    ...
    def test_the_ranking_was_done(self) -> None:
        ...
        levels = [m.level for m in DART_MAGNETS_2]
...
class OutputTests(unittest.TestCase):
    ...
    def test_a_different_correct_order_also_prints_the_answer(self) -> None:
        """The sealed puzzle's note says its declarations can sit in any
        order around main. Checked: main first, everything else after."""
        m = _find("magnet-dart-sealed-switch")
        pieces = list(m.pieces)
        moved = [pieces[-1], *pieces[:-1]]
        self.assertNotEqual(moved, list(m.pieces))
        self.assertTrue(same_pieces(m, moved))
        self.assertEqual(_run("\n".join(moved)), m.expect)

    def test_super_after_the_print_swaps_the_output(self) -> None:
        """The super puzzle's note, checked: the same lines with the super
        call below the subclass's own print."""
        m = _find("magnet-dart-super-call")
        pieces = list(m.pieces)
        sup = next(p for p in pieces if "super.open()" in p)
        at = pieces.index(sup)
        pieces[at], pieces[at + 1] = pieces[at + 1], pieces[at]
        self.assertEqual(_run("\n".join(pieces)),
                         "load Home data\nset up Home")

    def test_the_stream_body_waits_for_a_listener(self) -> None:
        """The stream puzzle's note says an async* body does not start
        when it is called. Moving 'stream made' above the call must not
        change the output - if it did, the note would be false."""
        m = _find("magnet-dart-await-for")
        pieces = list(m.pieces)
        made = next(p for p in pieces if "stream made" in p)
        call = next(p for p in pieces if "countdown(3)" in p)
        pieces.remove(made)
        pieces.insert(pieces.index(call), made)
        self.assertNotEqual(pieces, list(m.pieces))
        self.assertEqual(_run("\n".join(pieces)), m.expect)

    def test_the_setter_caps_the_value(self) -> None:
        """Swapping the two assignments swaps the output, as the note says."""
        m = _find("magnet-dart-getter-setter")
        pieces = list(m.pieces)
        a = pieces.index("  v.level = 7;")
        b = pieces.index("  v.level = 42;")
        pieces[a], pieces[b] = pieces[b], pieces[a]
        self.assertEqual(_run("\n".join(pieces)), "level 10\nlevel 7")
...
class RegisteredTests(unittest.TestCase):
    ...
    def test_the_family_is_served_with_its_language(self) -> None:
        ...
        rows = {row["id"]: row for row in family["magnets"]}
        for m in DART_MAGNETS_2:
            with self.subTest(magnet=m.id):
                self.assertIn(m.id, rows)
                self.assertEqual(rows[m.id]["language"], "dart")
                self.assertNotIn("plan", rows[m.id])
    ...
    @unittest.skipUnless(dart_available(), NEEDS_DART)
    def test_main_above_the_classes_is_marked_right(self) -> None:
        m = _find("magnet-dart-sealed-switch")
        pieces = list(m.pieces)
        self.assertTrue(
            self._check(m.id, [pieces[-1], *pieces[:-1]]).passed)
