"""The second set of Dart bug hunts, held to the rules every hunt keeps.
...
run over DART_HUNTS_2 directly, so the set is checked on its own
...
class CollectionTests(unittest.TestCase):
    ...
    def test_there_are_five_dart_hunts(self) -> None:
        self.assertEqual(len(DART_HUNTS_2), 5)
        for h in DART_HUNTS_2:
    ...
    def test_ids_are_unique_and_new(self) -> None:
        ...
        ids = [h.id for h in DART_HUNTS_2]
    ...
    def test_they_read_easiest_first_and_are_not_all_one_level(self) -> None:
        levels = [h.level for h in DART_HUNTS_2]
...
class TheReportIsTrueTests(unittest.TestCase):
    ...
    def test_the_mutation_hunt_is_caught_changing_its_argument(self) -> None:
        """The removeWhere hunt's bug is partly that the caller's list
        changes, which only the driver's before-and-after check sees."""
        h = next(h for h in DART_HUNTS_2 if h.id == "hunt-dart-remove-where")
        self.assertTrue(try_input(h, h.reported).changed)
