"""Every JavaScript solution is executed under node against real cases.

The Python bank has had this since it existed, and for the same reason: these
solutions are typed verbatim as muscle memory, so a wrong line would be
drilled in rather than caught. The JavaScript bank only had a spot check on
two patterns, which left most of it resting on "it isn't Python and it has
braces".

Each pattern is run once — its preamble, every solution in it, and a block of
assertions — so the cost is one node process per pattern rather than per
problem. The source executed is the exact string the student is asked to type.
"""
...
PATTERNS_BY_ID = {p.id: p for p in JS_PATTERNS}

# Shared by the checks below: a tiny assert that prints nothing when it passes,
# so a green run has empty stdout and a failure names itself.
HARNESS = """
let failed = [];
...
# pattern id -> (extra helper source, assertions)
...
const blank = Array.from({ length: 9 }, () => Array(9).fill('.'));
ok('isValidSudoku blank', isValidSudoku(blank));
const dup = Array.from({ length: 9 }, () => Array(9).fill('.'));
dup[0][0] = '5'; dup[1][1] = '5';
eq('isValidSudoku same box', isValidSudoku(dup), false);
"""),
    "lc-two-pointers": ("", """
...
const dedup = [0, 0, 1, 1, 1, 2, 2, 3, 3, 4];
eq('removeDuplicates', removeDuplicates(dedup), 5);
eq('removeDuplicates front', dedup.slice(0, 5), [0, 1, 2, 3, 4]);
...
eq('decodeString wide', decodeString('12[a]'), 'a'.repeat(12));
"""),
    "lc-linked-list": (LIST_HELPERS, """
eq('reverseList', readList(reverseList(makeList([1, 2, 3]))), [3, 2, 1]);
eq('mergeTwoLists', readList(mergeTwoLists(makeList([1, 2, 4]), makeList([1, 3, 4]))),
   [1, 1, 2, 3, 4, 4]);
...
eq('deleteDuplicates', readList(deleteDuplicates(makeList([1, 1, 2, 3, 3]))), [1, 2, 3]);
ok('isPalindromeList', isPalindromeList(makeList([1, 2, 2, 1])));
eq('isPalindromeList no', isPalindromeList(makeList([1, 2])), false);
eq('addTwoNumbers', readList(addTwoNumbers(makeList([2, 4, 3]), makeList([5, 6, 4]))),
   [7, 0, 8]);
eq('addTwoNumbers carry', readList(addTwoNumbers(makeList([9, 9]), makeList([1]))),
   [0, 0, 1]);
"""),
    "lc-binary-search": ("", """
...
eq('searchMatrix empty', searchMatrix([], 1), false);
"""),
...
eq('isSameTree shape', isSameTree(makeTree([1, 2, null]), makeTree([1, null, 2])), false);
ok('isSymmetric', isSymmetric(makeTree([1, 2, 2, 3, 4, 4, 3])));
eq('isSymmetric no', isSymmetric(makeTree([1, 2, 2, null, 3, null, 3])), false);
const lca = makeTree([3, 5, 1, 6, 2, 0, 8]);
ok('lowestCommonAncestor', lowestCommonAncestor(lca, lca.left, lca.right) === lca);
ok('lowestCommonAncestor self',
   lowestCommonAncestor(lca, lca.left, lca.left.right) === lca.left);
"""),
    "lc-tree-bfs": (TREE_HELPERS, """
eq('levelOrder', levelOrder(makeTree([3, 9, 20, null, null, 15, 7])),
   [[3], [9, 20], [15, 7]]);
eq('rightSideView', rightSideView(makeTree([1, 2, 3, null, 5, null, 4])), [1, 3, 4]);
...
eq('cloneGraph', cloneGraph(new GraphNode(1, [new GraphNode(2)])).val, 1);
eq('maxAreaOfIsland',
...
eq('pacificAtlantic one', pacificAtlantic([[1]]), [[0, 0]]);
"""),
    "lc-backtracking": ("", """
...
eq('findAllRecipes',
   ...
   ['bread']);
eq('findAllRecipes short', findAllRecipes(['bread'], [['yeast', 'flour']], ['yeast']), []);
...
eq('wordBreak no', wordBreak('catsandog', ['cats', 'dog', 'sand', 'and', 'cat']), false);
...
class JavaScriptSolutionTests(unittest.TestCase):
    """One node run per pattern, over the exact source the student types."""
    ...
    def test_every_pattern_is_covered(self) -> None:
        """A pattern with no checks would pass by not being looked at."""
        self.assertEqual(sorted(CHECKS), sorted(PATTERNS_BY_ID))

    def test_every_solution_is_exercised(self) -> None:
        """Each solution's function name has to appear in its pattern's
        checks, or it is shipped without ever having been run."""
        import re
        ...
        for pattern_id, pattern in PATTERNS_BY_ID.items():
            ...
            for problem in pattern.problems:
                found = re.search(
                    r"(?:^function (\w+)|^class (\w+))", problem.code, re.M
                )
                name = found.group(1) or found.group(2) if found else None
                if name and name not in checks:
                    missing.append(f"{pattern_id} #{problem.number} {name}")
        self.assertEqual(missing, [], "add a check for these")
...
def _make(pattern_id: str):
    ...
    test.__name__ = f"test_{pattern_id.replace('-', '_')}"
    test.__doc__ = f"Every solution in {pattern_id} runs and answers correctly."
    return test


for _pid in CHECKS:
    setattr(JavaScriptSolutionTests, f"test_{_pid.replace('-', '_')}", _make(_pid))
